...
(function (mod) {
  ...
  function validator (text, options) {
    if (!window.LNTLINT) {
      if (window.console) {
        window.console.error('Error: window.LNTLINT not defined, CodeMirror linting cannot run.')
      }
      return []
    }
    ...
    LNTLINT(text, options, options.globals)
    var errors = LNTLINT.data().errors; var result = []
    if (errors) parseErrors(errors, result)
    return result
  }

  CodeMirror.registerHelper('lint', 'lnt', validator)
